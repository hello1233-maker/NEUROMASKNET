...
class AVED_AAD_Dataset(Dataset):
    """
    AVED loader adapted to the exact model/training pipeline in the DTU code.

    Expected files:
        root/sub1.npz, root/sub2.npz, ..., root/sub10.npz

    Expected NPZ keys:
        eeg   : EEG segments
        label : binary attended-direction labels

    Each sample is returned as [32, 128].
    """
    def __init__(self, root, subject_ids, all_subject_ids_map):
        ...
        for s_id in subject_ids:
            file_path = os.path.join(root, f"sub{s_id}.npz")
            if not os.path.exists(file_path):
                warnings.warn(
                    f"Data file not found for subject {s_id}: {file_path}"
                )
                continue
            ...
            if 'label' not in data:
                raise KeyError(f"{file_path} does not contain key 'label'.")

            eeg_data = torch.tensor(data['eeg'], dtype=torch.float32)

            # Same per-sample min-max normalization used by the original DTU code.
    ...
    def __getitem__(self, idx):
        ...
        s_id = map_entry['subject_id']
        sample_idx = map_entry['sample_idx']

        subject_data = self.data_cache[s_id]
        eeg = subject_data['eeg'][sample_idx]

        # AVED code uses 32 channels x 128 samples.
        if eeg.dim() == 3:
            eeg = eeg.squeeze(0)

        if eeg.shape == (128, 32):
            eeg = eeg.transpose(0, 1)
        elif eeg.shape == (32, 128):
            pass
        elif eeg.numel() == 32 * 128:
            eeg = eeg.reshape(32, 128)
        else:
            raise ValueError(
                f"Unexpected AVED EEG shape for subject {s_id}, "
                f"sample {sample_idx}: {tuple(eeg.shape)}. "
                f"Expected (32,128) or (128,32)."
            )

        direction_label = subject_data['direction_label'][sample_idx]
        domain_label = self.domain_label_map.get(s_id, -1)

        if direction_label.item() not in (0, 1):
            raise ValueError(
                f"AVED label must be 0/1 for this binary classifier, "
                f"but got {direction_label.item()} for subject {s_id}, "
                f"sample {sample_idx}."
            )
...
def get_spatial_regions_32():
    """
    Same 10-region definition used by the AVED reference code.
    """
    return {
        "Pre-Frontal": ["FP1", "FP2"],
        "Frontal": ["FZ", "F3", "F4"],
        "Left Frontal-Temporal": ["F7", "FT7", "FC3"],
        "Right Frontal-Temporal": ["F8", "FT8", "FC4"],
        "Left Temporal-Central": ["T3", "C3", "TP7", "CP3"],
        "Right Temporal-Central": ["T4", "C4", "TP8", "CP4"],
        "Central": ["CZ", "FCZ", "CPZ"],
        "Left Parietal-Occipital": ["T5", "P3", "O1"],
        "Right Parietal-Occipital": ["T6", "P4", "O2"],
        "Midline Reference": ["PZ", "OZ", "A1", "A2"],
    }


def get_spatial_masks(device, num_channels=32):
    """
    Creates the 10 anatomical masks for the AVED 32-channel layout.
    """
    regions = get_spatial_regions_32()
    active_channels = CHANNEL_NAMES_32[:num_channels]
...
class TrainingConfig:
    def __init__(self):
        self.dataset_name = "AVED_NeuroMask_DTU_Model_32Ch"
        self.data_root = "/share/workspace/shared_datasets/AVED/audio-video/"
        self.save_dir = "./AVED_DTU_NeuroMask_results"
        ...
        # Training: kept the same as the original DTU model
        ...
        # Loss weights: unchanged from DTU model
        self.lambda_recon = 0.5
        self.alpha_ortho = 3.0
        self.gamma_suff = 0.2
        self.beta_kl = 0.001

        # AVED input dimensions
        self.channel_size = 32
        self.time_sample_num = 128
        self.patch_size = 16

        # Model dimensions: unchanged from DTU model
        self.spa_dim = 16
        self.emb_size = 128
        self.depth = 2
        self.num_heads = 8

        # AVED has 10 subjects.
        self.all_subject_ids = [str(i) for i in range(1, 11)]

        # Set to 0 for all 10 folds.
        # Set to 3 if you intentionally want to resume from subject 4.
...
def main():
    try:
        ...
        for i, test_subject_id in enumerate(subjects_to_process):
            ...
            train_dataset = AVED_AAD_Dataset(
                config.data_root,
                train_ids,
                domain_map
            )
            test_dataset = AVED_AAD_Dataset(
                config.data_root,
                [test_subject_id],
                domain_map
            )
        ...
        print("\n" + "=" * 80)
        print("All processed AVED LOSO folds completed!")
        print("=" * 80)
